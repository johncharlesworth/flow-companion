...
test('the header buttons, the model chip, the plus, and the gauge show their tooltips on hover', async ({ context, extensionId, serviceWorker }) => {
  ...
  // The gauge sits between the plus and Send on the bottom row, eight pixels from each neighbour, the same as the model chip is from the group.
  const [plusBox, sendBox] = [await panel.getByRole('button', { name: 'Quick actions' }).boundingBox(), await panel.getByRole('button', { name: 'Send' }).boundingBox()];
  expect(plusBox!.x - (chipBox!.x + chipBox!.width)).toBeGreaterThanOrEqual(7);
  expect(plusBox!.x - (chipBox!.x + chipBox!.width)).toBeLessThanOrEqual(9);
  expect(sendBox!.x - (plusBox!.x + plusBox!.width)).toBeGreaterThanOrEqual(7);
  expect(sendBox!.x - (plusBox!.x + plusBox!.width)).toBeLessThanOrEqual(9);
